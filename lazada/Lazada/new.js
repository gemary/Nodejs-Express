...
const THUECODEVN = require('./thuecodevn')
const {
    setIntervalAsync,
    clearIntervalAsync
...
async function sendVerifyCode(mobile, sessionID, apiToken) {
            ...
            if (success) {
                ...
                const getVerify = setIntervalAsync(async () => {
                    ...
                    if (end === 120000) {
                        ...
                        let {phoneNumber, content, status, timeout} = await THUECODEVN.getResult(sessionID, apiToken)
...
setImmediate(async () => {
    ...
    const apiToken = await getToken('thuecodevn-key.txt')

    if (apiToken === "") {
        console.log("Bạn chưa điền token")
    } else {
        const sessionID = await THUECODEVN.createRequest(apiToken)
        setTimeout(async () => {
            const {phoneNumber, content, status, timeout} = await THUECODEVN.getResult(sessionID, apiToken)
            if (status === "WAITINGPHONENO") {
                ...
                await sendVerifyCode(mobile, sessionID, apiToken)
                const {balance, number} = await THUECODEVN.getBalance(apiToken)
