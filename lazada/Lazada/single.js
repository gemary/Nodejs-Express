...
async function sendVerifyCode(mobile, sessionID, apiToken) {
    ...
    return new Promise(async (resolve, reject) => {
        await axios.post(url, payload, {
            ...
            if (success) {
                const tmpVerifyCode = await prompts({
                    type: 'text',
                    name: 'code',
                    message: 'Xin mời nhập mã OTP: ',
                    validate: value => value === "" ? `Bạn chưa nhập mã OTP` : true

                })

                await registerByOTP(mobile, tmpVerifyCode.code, umidToken, csrfToken, cookie)
            }
        }).catch((err) => {
...
setImmediate(async () => {
    ...
    const apiToken = await getToken('token.txt')
    ...
    if (apiToken === "") {
        ...
        const response = await prompts({
            ...
            validate: value => value !== "" ? `Bạn chưa nhập số điện thoại` : true
        });

        await sendVerifyCode(response.number, sessionID, apiToken)
        const {balance, number} = await OTPSIM.getBalance(apiToken)
